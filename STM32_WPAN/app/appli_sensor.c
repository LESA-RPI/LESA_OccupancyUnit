...
//  PRESSURE_StatusTypeDef tempStatus = PRESSURE_OK;
  ...
  {
  case INITIAL_CIE_1931_CHROMATICITY_COORDINATES_PID:
  {
	  #if (0) //TMD3725
	  ...
	  //clear
	  pValueParams->data[1] = sensor.colorinfo[1];
      pValueParams->data[0] = sensor.colorinfo[0];
      //colors
	  pValueParams->data[2] = sensor.colorinfo[2];
      pValueParams->data[3] = sensor.colorinfo[3];
	  pValueParams->data[4] = sensor.colorinfo[4];
      pValueParams->data[5] = sensor.colorinfo[5];
	  pValueParams->data[6] = sensor.colorinfo[6];
      pValueParams->data[7] = sensor.colorinfo[7];

	  TRACE_M(TF_SENSOR_M, "0x%.2x%.2x 0x%.2x%.2x 0x%.2x%.2x 0x%.2x%.2x    0x%.2x\n\r",
			  sensor.colorinfo[1],
			  sensor.colorinfo[0],
			  sensor.colorinfo[3],
			  sensor.colorinfo[2],
	  ...
	  //clear
	  pValueParams->data[1] = sensor.XYZinfo[1];
      pValueParams->data[0] = sensor.XYZinfo[0];
      //colors
	  pValueParams->data[2] = sensor.XYZinfo[2];
      pValueParams->data[3] = sensor.XYZinfo[3];
	  pValueParams->data[4] = sensor.XYZinfo[4];
      pValueParams->data[5] = sensor.XYZinfo[5];
	  pValueParams->data[6] = sensor.XYZinfo[6];
      pValueParams->data[7] = sensor.XYZinfo[7];
/*
	  TRACE_M(TF_SENSOR_M, "0x%.2x%.2x 0x%.2x%.2x 0x%.2x%.2x 0x%.2x%.2x    0x%.2x\n\r",
			  sensor.XYZinfo[1],
			  sensor.XYZinfo[0],
  ...
  case TIME_OF_FLIGHT_PID:
  {
	  ...
	  for(int i = 0; i<64;i++){
		  ...
		  pValueParams->data[i*2]=(uint8_t)temp;
		  pValueParams->data[i*2+1]=(uint8_t)(temp>>8);
		  //TRACE_M(TF_SENSOR_M, "%d\n\r",i);
	  }
	  //TRACE_M(TF_SENSOR_M, "head: 0x%.2x  %.2x\n\r",pValueParams->data[127],pValueParams->data[126]);
	  //pValueParams=&(Result.ZoneResult);
	  break;
  }
  }
...
void Appli_Sensor_ReadValue_UART()
{
	  uint8_t datas[6+128];
	  tcs3430_optics_val color_data;
	  tcs3430 sensor = { {0}, {0} };
	  ...
	  write(1,&AckByte,1);
	  write(1,datas,6+128);
//	  HAL_UART_Transmit(stdout,datas,6+128,1000);
}
...
//  int32_t pressure;
  ...
  if(ENABLE_SENSOR_MODEL_SERVER){
	  TRACE_M(TF_SENSOR, "VL53L8CX init");
	  MX_TOF_Init();
	  TRACE_M(TF_SENSOR, "TCS3430 init");
	  Color_Init();
	  /*
	  TRACE_M(TF_SENSOR, "TMD3725 init");
	  Color_Init();*/
