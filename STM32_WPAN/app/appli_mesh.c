...
#define LOOP_TIME_INTERVAL				(50000/CFG_TS_TICK_VAL) /*50ms*/
#if (APPLI_OPTIM == 1)
...
enum ButtonState {
	BS_OFF, BS_DEBOUNCE, BS_SHORT_PRESS, BS_LONG_PRESS
};
...
const MOBLEUINT8 PubKeyBuff[64] = { 0xF4, 0x65, 0xE4, 0x3F, 0xF2, 0x3D, 0x3F,
		0x1B, 0x9D, 0xC7, 0xDF, 0xC0, 0x4D, 0xA8, 0x75, 0x81, 0x84, 0xDB, 0xC9,
		0x66, 0x20, 0x47, 0x96, 0xEC, 0xCF, 0x0D, 0x6C, 0xF5, 0xE1, 0x65, 0x00,
		0xCC, 0x02, 0x01, 0xD0, 0x48, 0xBC, 0xBB, 0xD8, 0x99, 0xEE, 0xEF, 0xC4,
		0x24, 0x16, 0x4E, 0x33, 0xC2, 0x01, 0xC2, 0xB0, 0x10, 0xCA, 0x6B, 0x4D,
...
/* 32 octets Private Key information to be input here. It is only required for Public Key OOB case.
 Used during provisioning by Library */
const MOBLEUINT8 PrivKeyBuff[32] = { 0x52, 0x9A, 0xA0, 0x67, 0x0D, 0x72, 0xCD,
		0x64, 0x97, 0x50, 0x2E, 0xD4, 0x73, 0x50, 0x2B, 0x03, 0x7E, 0x88, 0x03,
		0xB5, 0xC6, 0x08, 0x29, 0xA5, 0xA3, 0xCA, 0xA2, 0x19, 0x50, 0x55, 0x30,
		0xBA };
#else
...
#if PB_ADV_SUPPORTED
MOBLEUINT8 discoverTimer_Id, discoverTimer_Id2;
...
static uint8_t BaseFrameValid = 0;
...
static void Appli_ShortButtonPress(void) {
	/* CIE color PID */
	pPropertyId[0] = 0x1D;    // Property ID byte 0 : Property ID for the sensor
	pPropertyId[1] = 0x00;    // Property ID byte 1 : Property ID for the sensor
	//MESH_MODEL_SENSOR_APP_CODE: Begin
#if ENABLE_SENSOR_MODEL_CLIENT==1
	TRACE_M(TF_SENSOR,"\n\rClient Short Button Pressed\r\n");
	...
	#elif ENABLE_SENSOR_MODEL_SERVER==1
	TRACE_M(TF_SENSOR, "\n\rServer Short Button Pressed\r\n");
//length 2, peer 0004, dst 0003, element index 0
	Sensor_Get(pPropertyId, 2, 0xc000, 0xc000, 0);//TODO: change the master address dynamically
...
static void Appli_ShortButton2Press(void) {
	...
	if (Sensor_Running) {
		HW_TS_Stop(discoverTimer_Id2);
		MX_TOF_Stop();
		Sensor_Running = 0;
	} else {
		MX_TOF_Begin();
		HW_TS_Start(discoverTimer_Id2, LOOP_TIME_INTERVAL);
		Sensor_Running = 1;
	}
...
static void Appli_UpdateButton2State(int isPressed) {
	...
	t0 = Clock_Time(); /* press timing */
	while (BSP_PB_GetState(BUTTON_SW2) == BUTTON_PRESSED)
		;	//TODO: fill out this
	t1 = Clock_Time(); /* SW2 release timing */
...
/*
	...
	//task 4
	Update_ALSThreshold(sensor.reginfo);
	reset_Status(sensor.reginfo);
	MX_TOF_Stop();
...
MOBLE_RESULT Appli_BleSetProductInfoCB(MOBLEUINT8 *company_product_info) {
	...
	memcpy(company_product_info + 4, (void*) &product_version_id,
...
MOBLEUINT8 Appli_BleSetNumberOfElementsCb(void) {
	if (NumberOfElements > BLEMesh_GetNumberOfElements()) {
		TRACE_M(TF_MISC,
				"Number of Elements enabled in application exceeding from Library Capability!\r\n");
	...
	if (BSP_PB_GetState(BUTTON_SW1) == BUTTON_PRESSED) {
		...
		if (!interrupted) {
			...
			TRACE_M(TF_PROVISION,
					"Device is unprovisioned by application \r\n");
			t = Clock_Time();
...
#ifdef INTERNAL_UNIQUE_NUMBER_MAC
	status = aci_hal_read_config_data(CONFIG_DATA_PUBADDR_OFFSET, &bdaddr[7],
			bdaddr);
 ...
 * @retval MOBLE_RESULT
 */
MOBLE_RESULT BLEMesh_ScanDevices(neighbor_params_t *unprovDeviceArray,
		MOBLEUINT8 *noOfUnprovDevices) {
	MOBLE_RESULT result;
 ...
 * @retval MOBLE_RESULT
 */
MOBLE_RESULT BLEMesh_ProvisionDevice(neighbor_params_t *unprovDeviceArray,
		MOBLEUINT16 index) {
...
void BLEMesh_FnFriendshipEstablishedCallback(MOBLE_ADDRESS lpnAddress,
		MOBLEUINT8 lpnReceiveDelay, MOBLEUINT32 lpnPollTimeout,
		MOBLEUINT8 lpnNumElements, MOBLE_ADDRESS lpnPrevFriendAddress) {
	TRACE_M(TF_LPN_FRND,
			"Friendship established. Low power node address 0x%.4X \r\n",
			lpnAddress);
	...
	if (lpnPrevFriendAddress != MOBLE_ADDRESS_UNASSIGNED) {
		TRACE_M(TF_LPN_FRND,
				"Low power node previous friend address 0x%.4X \r\n",
...
void BLEMesh_FnFriendshipClearedCallback(MOBLEUINT8 reason,
	...
	TRACE_M(TF_LPN_FRND,
			"Friendship cleared. Low power node address 0x%.4X \r\n",
			lpnAddress);

	switch (reason) {
	case FN_CLEARED_REPEAT_REQUEST:
		TRACE_M(TF_LPN_FRND,
				"Reason: New friend request received from existing low power node \r\n");
		break;
	case FN_CLEARED_POLL_TIMEOUT:
		TRACE_M(TF_LPN_FRND,
				"Reason: Low power node poll timeout occurred \r\n");
		break;
...
void BLEMesh_NeighborAppearedCallback(const MOBLEUINT8 *bdAddr,
		MOBLEBOOL provisioned, const MOBLEUINT8 *uuid,
		MOBLE_ADDRESS networkAddress, MOBLEINT8 rssi) {
...
static void AppliMeshSW2Task(void) {
	Appli_UpdateButton2State(BSP_PB_GetState(BUTTON_SW2) == BUTTON_PRESSED);
	return;
}
static void TrackingTask(void){
if(BaseFrameValid&&BSP_PB_GetState(COLOR_INT) == BUTTON_PRESSED){
	beginTracking();}
	return;
}
 ...
 * @retval void
 */
void Appli_Init(MOBLEUINT8 *flag) //TODO: add task seq
{
#if 0
...
/* Hardware Factory Restore implementation */
...
#if PB_ADV_SUPPORTED
	UTIL_SEQ_RegTask(1 << CFG_TASK_MESH_BEACON_REQ_ID, UTIL_SEQ_RFU,
			UnprovisionedDeviceBeaconApp);
	/**
	 * Create Timer to control unprovisioned device beacons
	 */
	HW_TS_Create(CFG_TIM_PROC_ID_ISR, &discoverTimer_Id, hw_ts_SingleShot,
			UnprovisionedDeviceBeaconTask);
	HW_TS_Start(discoverTimer_Id, DISCOVER_TIMER_INTERVAL);
	/*
	 * Create Timer to control interrupt//TODO
	 * */
	UTIL_SEQ_RegTask(1 << CFG_TASK_USER_SET_TASK, UTIL_SEQ_RFU, LoopApp);

	HW_TS_Create(CFG_TIM_PROC_ID_ISR, &discoverTimer_Id2, hw_ts_Repeated,
			LoopTask);
  ...
  Modify_PWM(BLUE_LED, 1);
	...
	UTIL_SEQ_RegTask(1 << CFG_TASK_MESH_SW1_REQ_ID, UTIL_SEQ_RFU,
			AppliMeshSW1Task);
	UTIL_SEQ_RegTask(1 << CFG_TASK_MESH_SW2_REQ_ID, UTIL_SEQ_RFU,
			AppliMeshSW2Task);
	UTIL_SEQ_RegTask(1 << CFG_TASK_MESH_COLOR_REQ_ID, UTIL_SEQ_RFU,
			TrackingTask);
...
#endif
#if (ENABLE_SENSOR_MODEL_SERVER != 0)
	UTIL_SEQ_RegTask(1 << CFG_TASK_MESH_SW3_REQ_ID, UTIL_SEQ_RFU,
			AppliMeshSW3Task);
#endif
 ...
 * @retval MOBLEUINT8
 */
MOBLEUINT16 PwmValueMapping(MOBLEUINT16 setValue, MOBLEUINT16 maxRange,
		MOBLEINT16 minRange) {
