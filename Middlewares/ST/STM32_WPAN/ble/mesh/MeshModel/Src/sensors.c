                                      ...
                                      MOBLEUINT16 pidNotFound)
{
  ...
  TRACE_M(TF_SENSOR_M, "Sensor_MarshalPid called\r\n");
                                  ...
                                  status_send_e statusSend)
{
  ...
  {    
    ...
    else
    {
      ...
      for (MOBLEUINT8 count=0; count<sensorCount; count++)
      {
        ...
        TRACE_M(TF_SENSOR_M, "Valid arguments\r\n");
  ...
  if (result == MOBLE_RESULT_SUCCESS)
  {
        ...
        statusSend == STATUS_SEND_REPLY_PUBLISH)
    {
      if (dstAddr != MOBLE_ADDRESS_UNASSIGNED)
      {
        TRACE_M(TF_SENSOR_M, "Reply about to send to %d\r\n",dstAddr);
        ...
        if (result == MOBLE_RESULT_SUCCESS)
        {
          TRACE_M(TF_SENSOR_M, "Reply sent to %d\r\n",dstAddr);
        }
        else
        {
          TRACE_M(TF_SENSOR_M, "Reply failed\r\n");
...
{  
  ...
  TRACE_M(TF_SENSOR_M, "param 0x%.4x length %ld, peer 0x%.4x, dst 0x%.4x, element index %d \r\n",
          *pGetParam, length, peerAddr, dstPeer, elementIndex);
  ...
  if (MOBLE_SUCCEEDED(result))
  {
    ...
    {
      /*Application Callback *///TODO: not package
      if(length == 2){
