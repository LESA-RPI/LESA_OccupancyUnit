...
// added to copy sensor data from sensor_client.c to main.c
// 21 length due to its 2 byte identifier and 19 byte data
uint8_t occ_sens_server_byte_stream[21];
...
void example_ble_mesh_send_sensor_message(uint32_t opcode)
{
    ...
    // switch statement for casdence, setting, and series removed
                                              ...
                                              esp_ble_mesh_sensor_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_SENSOR_CLIENT_GET_STATE_EVT:
        switch (param->params->opcode) {
        // removed excess cases not relevant to this code - could be in the future, refer to BLE_Mesh folder for original
        // this is where server data is output
        case ESP_BLE_MESH_MODEL_OP_SENSOR_GET:
            ...
            if (param->status_cb.sensor_status.marshalled_sensor_data->len) {
                // prints 48 bit (6 byte) hex string 
                ...
                memcpy(&occ_sens_server_byte_stream, data, 21);
                //printf("%d\n",param->status_cb.sensor_status.marshalled_sensor_data->len);
                uint16_t length = 0;
    ...
    case ESP_BLE_MESH_SENSOR_CLIENT_SET_STATE_EVT:
        switch (param->params->opcode) {
        // cases not relevant to this code removed
        default:
...
//takes in array from main.c and copies occ_sens_server_byte_stream to it
void occ_sens_client_ble_msg_get(uint8_t* array){
    //example_ble_mesh_sensor_client_cb(ESP_BLE_MESH_MODEL_OP_SENSOR_GET, )
    static uint32_t opcode = ESP_BLE_MESH_MODEL_OP_SENSOR_GET;
    example_ble_mesh_send_sensor_message(opcode);
    
    memcpy(array, &occ_sens_server_byte_stream, 21);
}
