...
void getOccupantData(float* output, struct Occupant person_t1, struct Occupant person_t2){
    ...
    int delta_x = col_t2 - col_t1;
    int delta_y = row_t2 - row_t1;

    printf("\n\n\n%d\n\n\n",delta_x);
    
    //int dist = pow(delta_x,2); //+ pow(delta_y,2);
    //dist = sqrt(dist);
...
void exampleRun(float *output, int (*control)[8],int (*firstData)[8],int (*lastData)[8]){
    ...
    int** norm_data1 = malloc(8 * sizeof(int*));
    int** norm_data2 = malloc(8 * sizeof(int*));
    for(int i = 0; i<8;i++){
        norm_data1[i] = malloc(8 * sizeof(int));
        norm_data2[i] = malloc(8 * sizeof(int));
    }
    ...
    getOccupantData(output, frame1,frame2);
    ...
    float height = norm_data2[frame2.location[0].row][frame2.location[0].col];

    free(norm_data1);
    free(norm_data2);

    //Prints the resulted occupant data
    output[0] = frame2.location[0].row;
    output[1] = frame2.location[0].col;
    output[4] = height;
    // printf("\n+++++++++++++++++++++++++++++++++++++\n");

    //parameter = (*float) dataArray;
